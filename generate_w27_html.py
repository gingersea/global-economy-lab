#!/usr/bin/env python3
"""Generate Week 27 prediction HTML with:
   1) 上周总结 - compare W25 prediction vs W25 actual
   2) 上月总结 - compare W23 prediction vs ~1 month actual
   3) 本周预测 - W27 prediction
"""
...
with open("output/weekly_prediction.json") as f:
    w27_data = json.load(f)

# Last week's prediction = W25 (from 2026-06-21 archive)
with open("output/archive/2026-06-21/weekly_prediction.json") as f:
    w25_data = json.load(f)

# 4-week-ago prediction = W23 (from 2026-06-02 archive)
with open("output/archive/2026-06-02/weekly_prediction.json") as f:
    w23_data = json.load(f)

w25_markets = {m["market"]: m for m in w25_data["markets"]}
w23_markets = {m["market"]: m for m in w23_data["markets"]}
w27_markets = {m["market"]: m for m in w27_data["markets"]}
...
def get_ytd(ticker, as_of="2026-06-30"):
    try:
        data = yf.download(ticker, start="2026-01-01", end=as_of, progress=False, auto_adjust=True)
        if data.empty or len(data) < 2:
            return None
        close_start = data["Close"].iloc[0].item()
        close_end = data["Close"].iloc[-1].item()
...
def get_monthly_change(ticker, start="2026-06-01", end="2026-06-30"):
    """Get % change over ~1 month period."""
    try:
        data = yf.download(ticker, start=start, end=end, progress=False, auto_adjust=True)
        if data.empty or len(data) < 2:
            return None
        ...
        print(f"  Error fetching monthly for {ticker}: {e}")
        return None

# ════════════════════════════════════════════
# Step 1: W25 review (Last week review)
# Compare W25 prediction (archive 2026-06-21) vs actual W25 results
# ════════════════════════════════════════════
print("=" * 60)
print("Step 1: Fetching actual data for W25 review (Week 25: 6/15-6/21)")
print("=" * 60)

w25_actual_start = "2026-06-12"
w25_actual_end = "2026-06-22"

w25_actual = {}
for market_code, ticker in TICKER_MAP.items():
    if market_code in w25_markets:
        pct, last_price = get_weekly_change(ticker, w25_actual_start, w25_actual_end)
        if pct is not None:
            direction = "UP" if pct > 0.2 else ("DOWN" if pct < -0.2 else "FLAT")
        else:
            direction = "N/A"
        w25_actual[market_code] = {"pct": pct, "direction": direction}
        print(f"  {market_code} ({ticker}): {pct}% → {direction}")

# Compare W25 predictions vs actual
print("\nComparing W25 predictions vs actual...")
T1_correct = 0
T1_total = 0
T2_correct = 0
T2_total = 0
...
for market_code, pred in w25_markets.items():
    actual = w25_actual.get(market_code, {"pct": None, "direction": "N/A"})
    signal = pred["signal"]
...
w25_T1_rate = round(T1_correct / T1_total * 100, 1) if T1_total > 0 else 0
w25_T2_rate = round(T2_correct / T2_total * 100, 1) if T2_total > 0 else 0
w25_total_correct = T1_correct + T2_correct
w25_total_compared = T1_total + T2_total

print(f"\nW25 Tier 1: {T1_correct}/{T1_total} ({w25_T1_rate}%)")
print(f"W25 Tier 2: {T2_correct}/{T2_total} ({w25_T2_rate}%)")

# ════════════════════════════════════════════
# Step 2: Monthly review (W23 prediction vs ~1 month actual)
# ════════════════════════════════════════════
print("\n" + "=" * 60)
print("Step 2: Fetching actual data for monthly review (W23 prediction vs June actual)")
print("=" * 60)

monthly_actual = {}
for market_code, ticker in TICKER_MAP.items():
    if market_code in w23_markets:
        pct = get_monthly_change(ticker, "2026-06-01", "2026-06-30")
        if pct is not None:
            direction = "UP" if pct > 0.5 else ("DOWN" if pct < -0.5 else "FLAT")
        else:
            direction = "N/A"
        monthly_actual[market_code] = {"pct": pct, "direction": direction}
        print(f"  {market_code} ({ticker}) monthly: {pct}% → {direction}")

# Compare W23 predictions vs monthly actual
print("\nComparing W23 predictions vs June actual...")
M_T1_correct = 0
M_T1_total = 0
M_T2_correct = 0
M_T2_total = 0
...
for market_code, pred in w23_markets.items():
    actual = monthly_actual.get(market_code, {"pct": None, "direction": "N/A"})
    signal = pred["signal"]
    actual_dir = actual["direction"]
...
M_T1_rate = round(M_T1_correct / M_T1_total * 100, 1) if M_T1_total > 0 else 0
M_T2_rate = round(M_T2_correct / M_T2_total * 100, 1) if M_T2_total > 0 else 0
M_total_correct = M_T1_correct + M_T2_correct
M_total_compared = M_T1_total + M_T2_total

print(f"\nMonthly Tier 1: {M_T1_correct}/{M_T1_total} ({M_T1_rate}%)")
print(f"Monthly Tier 2: {M_T2_correct}/{M_T2_total} ({M_T2_rate}%)")

# ════════════════════════════════════════════
# Step 3: Fetch YTD and weekly data for W27
# ════════════════════════════════════════════
print("\n" + "=" * 60)
print("Step 3: Fetching YTD data as of 2026-06-30")
print("=" * 60)

ytd_values = {}
for market_code, ticker in TICKER_MAP.items():
    if market_code in w27_markets:
        ytd = get_ytd(ticker, as_of="2026-06-30")
        ytd_values[market_code] = ytd
        print(f"  {market_code}: YTD {ytd}%")

print("\nFetching W27 weekly data as of 6/29-...")
w27_start = "2026-06-27"
w27_end = "2026-07-01"
w27_weekly = {}
for market_code, ticker in TICKER_MAP.items():
    if market_code in w27_markets:
        pct, _ = get_weekly_change(ticker, w27_start, w27_end)
        w27_weekly[market_code] = pct
        print(f"  {market_code}: week-to-date {pct}%")
...
def result_html(result, actual_pct):
    if result == "correct":
        return '<span style="color:#4caf50">✅ 正确</span>'
    elif result == "wrong":
        return '<span style="color:#f44336">❌ 错误</span>'
    elif result == "passive":
        return '<span style="color:#888">─ 无方向</span>'
    else:
...
# ════════════════════════════════════════════
# Generate W25 review table rows
# ════════════════════════════════════════════
def make_review_rows(results):
    review_rows = ""
...
def make_accuracy_blocks(results):
    ...
    # Correct block
    if correct_list:
        blocks += f'<div class="acc-block"><h4 style="color:#4caf50">✓ 预测正确 ({len(correct_list)} 市场)</h4><div class="correct-list">'
        items = [f'<span class="tier-badge tier{r["tier"]}">T{r["tier"]}</span>{r["name_cn"]} {format_pct(r["actual_pct"])}' for r in correct_list]
        blocks += "<p>" + " &nbsp; ".join(items) + "</p></div></div>"
    
    # Wrong block
    if wrong_list:
        blocks += f'<div class="acc-block"><h4 style="color:#f44336">✗ 预测错误 ({len(wrong_list)} 市场)</h4><div class="wrong-list">'
        items = [f'<span class="tier-badge tier{r["tier"]}">T{r["tier"]}</span>{r["name_cn"]} {signal_html(r["signal"])} → {format_pct(r["actual_pct"])}' for r in wrong_list]
        blocks += "<p>" + " &nbsp; ".join(items) + "</p></div></div>"
    
    # Passive block
    if passive_list:
        blocks += f'<div class="acc-block"><h4 style="color:#ff9800">─ 无方向信号 ({len(passive_list)})</h4><div style="font-size:12px;line-height:1.6;color:#888">'
        items = [f'<span class="tier-badge tier{r["tier"]}">T{r["tier"]}</span>{r["name_cn"]} NEUT → {format_pct(r["actual_pct"])}' for r in passive_list]
        blocks += "<p>" + " &nbsp; ".join(items) + "</p></div></div>"
    
    # Nodata block
    if nodata_list:
        blocks += f'<div class="acc-block"><h4 style="color:#666">? 数据缺失 ({len(nodata_list)})</h4><div style="font-size:12px;line-height:1.6;color:#888">'
        items = [f'<span class="tier-badge tier{r["tier"]}">T{r["tier"]}</span>{r["name_cn"]}' for r in nodata_list]
        blocks += "<p>" + " &nbsp; ".join(items) + "</p></div></div>"
    
    return blocks, correct_list, wrong_list, passive_list, nodata_list

# ════════════════════════════════════════════
# Generate W25 review HTML section
# ════════════════════════════════════════════
w25_review_rows = make_review_rows(w25_results)
w25_acc_blocks, w25_correct, w25_wrong, w25_passive, w25_nodata = make_accuracy_blocks(w25_results)

if w25_T1_rate >= 80:
    w25_insight_color = "#4caf50"
    w25_insight_title = "▸ 上周总结：预测表现优秀"
    w25_insight_text = "EPU 高企环境下趋势延续模式表现优异，模型对市场方向的判断保持稳健。"
elif w25_T1_rate >= 60:
    w25_insight_color = "#ff9800"
    w25_insight_title = "▸ 上周总结：预测表现一般"
    w25_insight_text = "EPU 高企环境下预测不确定性增大，部分市场出现反向走势。"
else:
    w25_insight_color = "#f44336"
    w25_insight_title = "▸ 上周总结：预测表现不及预期"
    w25_insight_text = "高 EPU 环境加剧市场异常波动，模型预测精准度下降。"

w25_insight_rgb = "76,175,80" if w25_T1_rate >= 80 else ("247,151,30" if w25_T1_rate >= 60 else "244,67,54")

# ════════════════════════════════════════════
# Generate Monthly review HTML section
# ════════════════════════════════════════════
monthly_review_rows = make_review_rows(monthly_results)
monthly_acc_blocks, m_correct, m_wrong, m_passive, m_nodata = make_accuracy_blocks(monthly_results)

if M_T1_rate >= 80:
    m_insight_color = "#4caf50"
    m_insight_title = "▸ 月度总结：长期趋势判断准确"
    m_insight_text = "模型对月度级别的趋势判断保持较高准确度，EPU 趋势延续模式在月维度上表现稳定。"
elif M_T1_rate >= 60:
    m_insight_color = "#ff9800"
    m_insight_title = "▸ 月度总结：趋势判断基本准确"
    m_insight_text = "部分市场出现中期反转，但整体方向判断仍保持合理准确率。"
else:
    m_insight_color = "#f44336"
    m_insight_title = "▸ 月度总结：月度预测精准度有待提升"
    m_insight_text = "高 EPU 持续环境下月度预测面临更大挑战，需关注结构性变化信号。"

m_insight_rgb = "76,175,80" if M_T1_rate >= 80 else ("247,151,30" if M_T1_rate >= 60 else "244,67,54")

# ════════════════════════════════════════════
# Generate W27 Tier 1 & Tier 2 table rows
# ════════════════════════════════════════════
tier1_markets = sorted([m for m in w27_data["markets"] if m["tier"] == 1],
...
for m in tier1_markets:
    ...
    week_chg = w27_weekly.get(m["market"])
...
tier2_markets = sorted([m for m in w27_data["markets"] if m["tier"] == 2],
                       key=lambda x: x["accuracy"], reverse=True)
...
max_ytd = max(abs(v) for v in ytd_values.values() if v is not None) if ytd_values else 100

ytd_bars = ""
for market_code, ytd in ytd_sorted:
    if ytd is None:
        continue
    name = w27_markets.get(market_code, {}).get("name_cn", market_code)
    if not name:
        continue
...
# ════════════════════════════════════════════
# W27 key judgments - overheat markets
# ════════════════════════════════════════════
overheat_items = []
all_markets = w27_data["markets"]
for m in all_markets:
    if m["tm_z"] > 2.0 and m["mr_z"] < -2.0:
        overheat_items.append(f'<li><span style="color:#f44336">{m["name_cn"]}</span>：趋势 {m["tm_z"]:+.2f}σ, 反转 {m["mr_z"]:+.2f}σ — <strong>极度超买</strong></li>')
    elif m["tm_z"] > 1.5 and m["mr_z"] < -1.5:
        overheat_items.append(f'<li><span style="color:#f44336">{m["name_cn"]}</span>：趋势 {m["tm_z"]:+.2f}σ, 反转 {m["mr_z"]:+.2f}σ — <strong>显著超买</strong></li>')
    elif m["tm_z"] > 1.0 and m["mr_z"] < -1.0:
        overheat_items.append(f'<li><span style="color:#ff9800">{m["name_cn"]}</span>：趋势 {m["tm_z"]:+.2f}σ, 反转 {m["mr_z"]:+.2f}σ — 趋于超买</li>')
    elif m["tm_z"] > 0.7:
        overheat_items.append(f'<li><span style="color:#ff9800">{m["name_cn"]}</span>：趋势 {m["tm_z"]:+.2f}σ — 趋势偏强</li>')

overheat_html = "\n".join(overheat_items) if overheat_items else "<li>暂无显著过热信号</li>"

# ════════════════════════════════════════════
# Build HK section for W27
# ════════════════════════════════════════════
hk_market = next((m for m in all_markets if m["market"] == "HK"), None)
hk_accuracy = "N/A"
hk_tier = "none"
if hk_market:
    hk_accuracy = f"{hk_market['accuracy'] * 100:.1f}"
    hk_tier = hk_market["tier"]
    hk_signal = hk_market["signal"]

hk_section_html = ""
if hk_market:
    hk_desc = f"""恒生指数为 Tier {hk_tier} 级别（准确率 {hk_accuracy}%），在 EPU 高企环境下延续趋势延续模式。
        虽信号为 BULL，但近期回调幅度较大，需密切关注地缘政治与流动性风险。"""
    if hk_tier == 1:
        hk_desc = f"""恒生指数已升级为 Tier 1 级别（准确率 {hk_accuracy}%），在 EPU 高企环境下延续趋势延续模式。
        BULL 信号较为可靠，但需关注中美政策边际变化。"""
...
# If HK is Tier 1, it won't normally be in the HK section since it's already in the Tier 1 table
# But we still have the special section for HK

# ════════════════════════════════════════════
# Assemble full HTML
# ════════════════════════════════════════════
generated_time = w27_data["generated_display"]
epu_val = w27_data["epu"]["value"]
epu_pct = w27_data["epu"]["percentile"]
china_epu = w27_data["epu"].get("china_epu", "N/A")
t1_ct = w27_data["tier1_count"]
t2_ct = w27_data["tier2_count"]

# W25 prediction period for review title
w25_period = w25_data["prediction_period"]
w23_period = w23_data["prediction_period"]
...
<title>全球市场预测 (第27周 6/29-7/5) · GingerFamily.CN</title>
...
<div class="container">
    ...
    <div class="page-header">
        ...
        <p class="sub">13 市场 · 2 因子模型 · EPU 制度切换 ｜ 预测周期 2026年第27周 (6/29-7/5) ｜ <span class="ts-stamp">生成 {generated_time}</span></p>
    </div>

    <!-- ═══════════════ 上周总结板块 ═══════════════ -->
    <h2 class="section-title">📊 上周预测回顾 (第25周 6/15-6/21)</h2>
    <div class="review-card">
        <p style="color:#888;font-size:13px;margin-bottom:16px">
            上周预测周期 {w25_period}，全部市场预测为 <span class="signal-bull">▲ BULL</span>，
            与本周 (6/15-6/21) 实际走势对比：
        </p>
        <div class="summary-stats">
            <div class="stat">
                <div class="n" style="color:{'#4caf50' if w25_T1_rate >= 70 else '#ff9800'}">{w25_T1_rate}%</div>
            ...
            <div class="stat">
                <div class="n" style="color:{'#4caf50' if w25_T2_rate >= 50 else '#f44336'}">{T2_correct}/{T2_total}</div>
                <div class="l">Tier 2 方向正确</div>
            </div>
            <div class="stat">
                <div class="n" style="color:{'#4caf50' if w25_total_correct/w25_total_compared >= 0.7 else '#ff9800'}">{w25_total_correct}/{w25_total_compared}</div>
        ...
        <div style="margin-top:16px;overflow-x:auto">
            <table class="predict-table" style="min-width:600px">
                ...
                <tbody>{w25_review_rows}
                </tbody>
            </table>
        </div>

        <div style="margin-top:20px;padding:16px;background:rgba({w25_insight_rgb},0.05);border-radius:10px;border:1px solid rgba({w25_insight_rgb},0.1)">
            <p style="font-size:13px;color:#aaa;margin:0">
                <span style="color:{w25_insight_color};font-weight:700">{w25_insight_title}</span>
                {w25_insight_text}
            </p>
        </div>
    </div>
    ...
    <div class="review-card">
        <p style="color:#888;font-size:13px;margin-bottom:16px">
            上月 (6/1) {w23_period} 预测全部市场为 <span class="signal-bull">▲ BULL</span>（CN为NEUT），
            对比 6 月整月实际走势 (6/1-6/30)，评估模型长期趋势判断准确度：
        </p>
        <div class="summary-stats">
            <div class="stat">
                <div class="n" style="color:{'#4caf50' if M_T1_rate >= 70 else '#ff9800'}">{M_T1_rate}%</div>
                <div class="l">Tier 1 月度正确率</div>
            </div>
            <div class="stat">
                <div class="n correct">{M_T1_correct}/{M_T1_total}</div>
                <div class="l">Tier 1 正确市场</div>
            </div>
            <div class="stat">
                <div class="n" style="color:{'#4caf50' if M_T2_rate >= 50 else '#f44336'}">{M_T2_correct}/{M_T2_total}</div>
                <div class="l">Tier 2 月度正确</div>
            </div>
            <div class="stat">
                <div class="n" style="color:{'#4caf50' if M_total_correct/M_total_compared >= 0.7 else '#ff9800'}">{M_total_correct}/{M_total_compared}</div>
                <div class="l">总体方向正确</div>
            </div>
        </div>
        ...
        <div style="margin-top:16px;overflow-x:auto">
            <table class="predict-table" style="min-width:600px">
                ...
                <tbody>{monthly_review_rows}
                </tbody>
            </table>
        </div>

        <div style="margin-top:20px;padding:16px;background:rgba({m_insight_rgb},0.05);border-radius:10px;border:1px solid rgba({m_insight_rgb},0.1)">
            <p style="font-size:13px;color:#aaa;margin:0">
                <span style="color:{m_insight_color};font-weight:700">{m_insight_title}</span>
                {m_insight_text}
            </p>
        </div>
    </div>
    ...
    <!-- ═══════════════ 本周预测板块 ═══════════════ -->
    <h2 class="section-title">🔮 本周预测 (第27周 6/29-7/5)</h2>
    ...
    <p style="font-size:12px;color:#666;margin-top:-8px;margin-bottom:8px">预测未来12个月收益方向 · YTD = 年初至今实际涨幅 · 周变 = 本周(6/29-7/5)变化 · 数据至 = 最近交易日</p>
    ...
    <h3 style="font-size:15px;color:#e0e0e0;margin-top:40px;margin-bottom:8px;">📊 YTD 表现对比 (截至 2026-06-30)</h3>
    ...
    <div class="card">
        <div class="grid-2">
            <div>
                ...
                <p style="font-size:13px;color:#aaa;line-height:1.7">
                    ...
                    Tier 1 全部 {t1_ct} 个市场 + Tier 2 全部 {t2_ct} 个市场均发出 BULL 信号。
                    这是 EPU 阈值效应下的典型表现：高不确定性环境掩盖了微弱反转信号，趋势因子权重提升。
                    中国EPU 为 {china_epu}，对恒生指数和中国A股的影响显著。
                </p>
            </div>
            <div>
        ...
        <div style="margin-top:20px">
            ...
            <p style="font-size:13px;color:#aaa;line-height:1.7">
                EPU 自 5 月以来维持在 350 附近（P90+），无回落迹象，当前百分位为 <strong>P{epu_pct}</strong>。
    ...
    <div class="insight-box" style="margin-top:32px">
        ...
        <strong>预测周期：2026年第27周 (6/29-7/5) · 生成时间：{generated_time}</strong></p>
    </div>

    <div style="text-align:center;margin-top:24px;font-size:11px;color:#444">
        <span class="ts-stamp">第27周存档</span>
        <span style="margin:0 8px">|</span>
        <span>上周准确率 Tier1: {w25_T1_rate}% ({T1_correct}/{T1_total}) | Tier2: {w25_T2_rate}% ({T2_correct}/{T2_total})</span>
        <span style="margin:0 8px">|</span>
        <span>月度准确率 Tier1: {M_T1_rate}% ({M_T1_correct}/{M_T1_total}) | Tier2: {M_T2_rate}% ({M_T2_correct}/{M_T2_total})</span>
        <span style="margin:0 8px">|</span>
...
with open("output/prediction.html", "w") as f:
    f.write(html)
print("\nWritten: output/prediction.html")

with open("output/prediction202627.html", "w") as f:
    f.write(html)
print("Written: output/prediction202627.html")

print("\nDone! HTML generation complete.")
